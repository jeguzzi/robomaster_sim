...
Server::Server(boost::asio::io_context& _io_context, short port)
...
std::vector<uint8_t> Server::answer_request(const uint8_t * buffer, size_t length)
{
  ...
  uint8_t set = buffer[9];
  uint8_t id = buffer[10];
  ...
  uint16_t seq_id = buffer[7] * 256 + buffer[6];
  uint8_t attri = buffer[8];
  uint8_t sender = buffer[4];
  uint8_t receiver = buffer[5];
  return callbacks.at(key)(sender, receiver, seq_id, attri, buffer+11);
}
