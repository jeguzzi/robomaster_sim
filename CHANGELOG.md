...
First version with binary releases on GitHub.

- Fixed linux linking options.
- Added option to build tar.gz archives using cpack.

## [1.3] - 2026-03-04

- Tested to support CoppeliaSim up to v4.10.
- Fixed IP in request for connection response.
- Fixed client updates (coherently with real robot): now only the (latest) client will receive updates. This prevents alternating between clients, which happened each time we received a message (e.g., an heartbeat).
